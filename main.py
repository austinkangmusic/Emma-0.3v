...
with open("transcription/user_input.txt", "w") as file:
    file.write('')
...
def file_to_transcribe(if_robotic):
    if if_robotic:
        apply_vocoder("audios/output_0.wav", "audios/vocoder_output.wav", "audios/carrier.wav")
...
def write_file_in_real_time(word_timestamps, output_file_path):
    ...
    current_transcription = ""  # To build the current transcription
    ...
    while last_word_index < len(word_timestamps):
        ...
        while (last_word_index < len(word_timestamps) and 
            ...
            with open(output_file_path, "w") as file:  # Open in write mode to overwrite
                file.write(current_transcription.strip())  # Write current transcription without trailing space
...
def chat_bot(conversation_history, user_input, halved_user_content, chatbot_listening, chatbot_response):
    ...
    while True:
        ...
        try:
            with open("transcription/user_input.txt", "r") as file:
        ...
        if tool_name in ('response', 'backchannel', 'interrupt'):
            ...
            try:
                with open("transcription/output_0.txt", "w") as file:
            ...
            output_file_path = "transcription/output_0.txt"
            ...
            if playback_active:
                ...
                while True:
                    ...
                    with open("transcription/output_0.txt", "r") as file:
                    ...
                    if not playback_active:
                        if spoken_user_input != '':
                            finished_speaking_system = f"You have finished speaking and at the same time, the user has overlapped your spoken words with: {spoken_user_input}"
